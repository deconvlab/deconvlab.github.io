...
ketex: true
---
In this article, we would like to elucidate several understandings toward the short-and-sparse deconvolution. Base on the knowledge of the existence of the symmetric solutions to this problem, its minimizing algorithm solver would be naturally non-convex, with the solutions being the local minima for the objective. These minimizers dictate the landscape of the whole geometry, and after realizing several distinct properties of this geometry, we are able to provide not only a provable result for short-and-sparse deconvolution, but more importantly, a better algorithm for solving the problem.

In the following, we will study an objective that approximates the effectively working lasso-type objective [$(2)$](/#formulation). Though not performing as well as the its origin, this approximation is easier to study, and gives us the insight how the landscape of objective is structured as well as how the minimization algorithm base on [$(2)$](/#formulation) would be performing. 
...
\\[ \min_{\mathbf a\in\mathbb S^{p-1},\, \mathbf x\in\mathbb R^{n}} \lambda \lVert \mathbf x\rVert_1 + \tfrac12  \lVert\mathbf a*\mathbf x - \mathbf y\rVert_2^2,  \\]
 ...
 then approximate this objective by substituting the bilinear convolution term $\lVert \mathbf a*\mathbf x\rVert_2^2$ as $\lVert \mathbf a \rVert_2^2 + \lVert\mathbf x\rVert_2^2$ with the cross termed in squared norm dropped. Since both $\lVert\mathbf a\rVert_2^2$ and $\lVert\mathbf y\rVert_2^2$ are constants, the simplified objective becomes 
...
The understanding of how the geometry of $\varphi_{\text{ABL}}$ is shaped by the shifts of $\mathbf a_0$ suggests it is possible to design an algorithm that guarantees solving short-and-sparse deconvolution. The algorithm is straight forward, which starts near one of the shifts subspaces, and stays near the subspace through the course of minimization.
...
{:.ref}
[1] Choudhary, Sunav, and  Mitra, Urbashi. "Sparse blind deconvolution: What cannot be done." *2014 IEEE International Symposium on Information Theory.* IEEE, 2014.
